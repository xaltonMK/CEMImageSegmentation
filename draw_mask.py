...
WRITE_PATH = './data/real_BosSegmentDeneme'
...
for batch_i in tqdm(range(generator.steps)):
    ...
    image = Image.new('RGB', (len(GT_mask[0]), len(GT_mask)), (0, 0, 0))
    blended = alpha_blend(np.array(image), GT_mask.astype(np.int32))
    
    blended = np.invert(blended)
