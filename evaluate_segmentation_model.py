...
for batch_i in tqdm(range(generator.steps)):
    ...
    if np.sum(GT_mask.astype(np.int32)) == 0:
        continue
    GT_mask = apply_white_threshold(original, GT_mask, WHITE_THRESH)
    overlap_50 = is_covering_segmentation(GT_mask.astype(np.int32), oneshot.astype(np.int32), 0.5)
    overlap_60 = is_covering_segmentation(GT_mask.astype(np.int32), oneshot.astype(np.int32), 0.6)
    overlap_70 = is_covering_segmentation(GT_mask.astype(np.int32), oneshot.astype(np.int32), 0.7)

    iou = get_IOU(GT_mask.astype(np.int32), oneshot.astype(np.int32))
    f1 = get_f1(GT_mask.astype(np.int32), oneshot.astype(np.int32))
    f1_sum += f1
    iou_sum += iou
    blended = alpha_blend(np.array(output), GT_mask.astype(np.int32))
    images_count += 1
